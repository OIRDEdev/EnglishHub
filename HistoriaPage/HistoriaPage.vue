<template>
  <div class="story-player-container" :class="{ 'dark-theme': isDarkTheme }">
    ...
    <div id="storyContainer" :class="[fontSize, { 'visible': isPlaying }]">
      <div id="transcript">
        <p>
          <span v-for="(sentence, index) in visibleSentences" :key="index" class="sentence_" :id="index">
          <span class="text_in_english" :id="'english-' + index">
            {{ sentence.original }}
          </span>
          <span class="translate_span" :id="'translation-' + index">
            {{ sentence.translation }}
            <button @click="sendSentence(index)">Add in Cards</button>
          </span>
        </span>
        </p>
      </div>
...
export default {
  ...
  methods: {
    ...
    sendSentence(index) {
      const englishText = document.getElementById(`english-${index}`)?.innerText || '';
      let translationtext = document.getElementById(`translation-${index}`)?.innerText || '';
      let translationText = translationtext.replace('Add in Cards', '').trim();
